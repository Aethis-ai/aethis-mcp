...
Decision tools and `aethis_discover_rulesets` (the public-catalogue browser) work with no API key. Tenant-scoped tools (`aethis_list_projects`, `aethis_list_rulesets`, all authoring) require an API key resolved from the referenced CLI profile or securely supplied process environment.
...
- [src/credentials.ts](src/credentials.ts) — paired Aethis credential/endpoint resolution (`resolveCredentials`; `resolveApiKey` is the key-only compatibility wrapper) and per-call LLM-key resolution (`resolveLlmKey`, keychain/env/raw forms).
...
- **The endpoint defaults to `https://api.aethis.ai`.** `AETHIS_PROFILE` pins the saved key/endpoint pair and absolute `XDG_CONFIG_HOME` locates CLI credentials. Explicit `AETHIS_BASE_URL` overrides the endpoint. MCP startup config faults refuse visibly; late login to a changed endpoint requires host restart.
...
- **Untrusted-content fencing (GHSA-ph7q-r9q4-922g, v0.5+; JSON-passthrough closed in v0.16 / aethis-mcp#45).** Tool output handed back to the LLM must wrap any API-supplied free-text field in an `<api_response>` fence. Two paths: (a) a **raw JSON passthrough** returns `okData(result)` (not `ok(fmt(result))`) — `fenceData` wraps the whole serialized blob in one fence with `UNTRUSTED_PREFACE`, so every leaf is covered at once; (b) a handler that **interpolates** a server-returned natural-language field into `lines.push(...)` fences that field with `fenceUntrusted(label, value)`. Identifier-shaped fields (IDs, enum values, weights) don't need the treatment. The deterministic gate is `tests/untrusted-fencing.test.ts`: it drives **every** tool with taint sentinels and fails if any free-text leaf escapes a fence — so a new tool that emits server free-text bare fails CI by design. Never reintroduce a bare `ok(fmt(result))`.
- **Tool capability annotations (v0.16, aethis-mcp#45).** Every tool's auth + mutation profile lives in the `TOOL_CAPABILITIES` registry (`src/index.ts`); `toolAnnotations(name)` derives the MCP `readOnlyHint`/`destructiveHint`/`openWorldHint` passed at registration. `tests/tool-capabilities.test.ts` enforces (1) no anonymous/no-key tool is mutating, and (2) the registry's `auth` matches which handlers actually call `requireAuth`. Add a new tool ⇒ add its `TOOL_CAPABILITIES` entry, its `tool-endpoint-map.ts` entry, and a `tests/untrusted-fencing.test.ts` case in the same change.
- **`server.json` + `tool-inventory.json` are generated, drift-guarded (v0.16, aethis-mcp#66).** `server.json` (the MCP Registry record) is derived from `package.json` — bump the version, then `npm run sync:server-json`; `check:server-json` fails CI on drift. `tool-inventory.json` is generated by `npm run gen:inventory` from the tool surface; `check:inventory` fails CI on drift. Both checks also run inside `npm test`. Release machinery: `scripts/build-release-artefact.mjs` (unprivileged tarball+SBOM+digest) and `.github/workflows/release.yml` (protected multi-environment publish). See [docs/RELEASE.md](docs/RELEASE.md).
