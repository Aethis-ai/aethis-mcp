...
function taintedClient(overrides: Record<string, unknown> = {}): AethisClient {
  ...
  const defaults: Record<string, unknown> = {
    ...
    replaceTests: vi.fn().mockResolvedValue({ added: 1, replaced: 1 }),
...
const CASES: Record<string, (h: ToolHandlers) => Promise<unknown>> = {
  ...
  aethis_set_tests: (h) => h.aethis_set_tests({ project_id: "p_1", test_cases: [{ name: "tc", field_values: {}, expected_outcome: "eligible" }] }),
      ...
      const rejecting = taintedClient(
        Object.fromEntries(
          [
            ...
            "archiveProject", "archiveRuleset", "createProject", "uploadSourceText", "addTests", "replaceTests",
